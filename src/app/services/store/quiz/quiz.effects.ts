...
import { CreateQuizRest, LoadQuizList, LoadQuizListRest } from './quiz.actions';
...
export class QuizEffects {
  ...
  createQuiz = createEffect(()=>{
    return this.actions$.pipe(
      ...
      switchMap((payload)=>{
        return this.http.post(environment.api+"/createquiz",{
          ...payload.quizInfo,
      ...
      switchMap(()=>{
        return this.http.get(environment.api+"getquizlist").pipe(
          mergeMap((response:any)=>{
            if(response.status){
              return [
                LoadQuizList({quizlist:response.data})
              ]
            }else{
              let toast = new Toast(response);
              toast.show=true;
